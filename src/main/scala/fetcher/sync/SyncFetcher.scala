...
class SyncFetcher(metric: Metric) {
  ...
  /**
    ...
    */
  def fetch(paths: List[String], peers: List[Peer]): Either[String, List[Snapshot]] =
    paths.foldLeft[Either[String, List[Snapshot]]](Right(Nil)) {
      case (l @ Left(_), _) => l
      case (Right(xs), path) => fetchOne(path, peers).map(_ :: xs)
    }

  private[sync] def fetchOne(path: String, peers: List[Peer]): Either[String, Snapshot] =
    peers.foldLeft[Either[String, Snapshot]](Left(s"Failed downloading $path")) {
      case (s @ Right(_), _) => s
